...
<body class="bg-gray-100 dark:bg-gray-900 font-sans antialiased">
            ...
            <nav class="p-4 space-y-2 overflow-y-auto h-[calc(100vh-4rem)]">
                ...
                <div class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <a href="{{ route('tenant.receivables.index') }}" 
                       class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('tenant.receivables.*') ? 'bg-primary text-white' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('tenant.payables.index') }}" 
                       class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('tenant.payables.*') ? 'bg-primary text-white' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('tenant.salaries.index') }}" 
                       class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('tenant.salaries.*') ? 'bg-primary text-white' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span class="font-medium">薪資管理</span>
                    </a>
                    ...
                    <div x-data="{ reportsOpen: {{ request()->routeIs('tenant.reports.*') ? 'true' : 'false' }} }">
                        <button @click="reportsOpen = !reportsOpen" 
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <span class="font-medium">財務報表</span>
                            </div>
                        ...
                        <!-- Reports Submenu -->
                        <div x-show="reportsOpen" x-collapse class="ml-4 mt-2 space-y-1">
                            <a href="{{ route('tenant.reports.financial-overview') }}" 
                               class="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition {{ request()->routeIs('tenant.reports.financial-overview') ? 'bg-primary text-white' : '' }}">
                            ...
                            <a href="{{ route('tenant.reports.ar-ap-analysis') }}" 
                               class="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition {{ request()->routeIs('tenant.reports.ar-ap-analysis') ? 'bg-primary text-white' : '' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('tenant.reports.project-profit-loss') }}" 
                               class="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition {{ request()->routeIs('tenant.reports.project-profit-loss') ? 'bg-primary text-white' : '' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('tenant.reports.payroll-labor') }}" 
                               class="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition {{ request()->routeIs('tenant.reports.payroll-labor') ? 'bg-primary text-white' : '' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700" x-data="{ settingsOpen: true }">
                    ...
                    <div x-show="settingsOpen" x-collapse class="ml-4 mt-2 space-y-1">
                        ...
                        <a href="{{ route('tenant.roles.index') }}" 
                           class="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition {{ request()->routeIs('tenant.roles.*') ? 'bg-gray-100 dark:bg-gray-700 font-medium' : '' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    (function() {
        ...
        function showError(el, msg) {
            ...
            p.className = 'field-error text-sm text-red-500 mt-1';
            p.textContent = msg || '此欄位為必填';
            el.parentElement.appendChild(p);
        }
        function getLabel(el) {
            var label = el.closest('.mb-4, .mb-6, .form-group, [class*="mb-"]');
            if (label) {
                var lbl = label.querySelector('label');
                if (lbl) return lbl.textContent.trim().replace(/\s*\*\s*$/, '').trim() + ' 為必填';
            }
            return '此欄位為必填';
        }
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('form').forEach(function (form) {
                // Skip search/filter forms (GET forms without data-validate)
                if (form.method.toLowerCase() !== 'post' && !form.dataset.validate) return;
                // Skip forms explicitly opted out
                if (form.dataset.novalidate === 'false') return;
                ...
                form.addEventListener('submit', function (e) {
                    ...
                    Object.keys(radioGroups).forEach(function (name) {
                        ...
                        // Remove old error
                        var container = group[0].closest('.mb-4, .mb-6, [class*="mb-"]');
                        if (container) {
