...
<head>
    ...
    <title>@yield('title', '超級管理員系統') - ECount</title>
        ...
        >
            ...
            <nav class="p-4 space-y-2 overflow-y-auto h-[calc(100vh-4rem)]">
                ...
                <a href="{{ route('superadmin.dashboard') }}" 
                   class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('superadmin.dashboard') ? 'bg-primary text-white' : '' }}">
                ...
                <a href="{{ route('superadmin.tenants.index') }}" 
                   class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('superadmin.tenants.*') ? 'bg-primary text-white' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('superadmin.plans.index') }}" 
                   class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('superadmin.plans.*') ? 'bg-primary text-white' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <a href="{{ route('superadmin.error-log.index') }}"
                       class="flex items-center space-x-3 px-4 py-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary hover:text-white transition {{ request()->routeIs('superadmin.error-log.*') ? 'bg-primary text-white' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-data="{ dropdownOpen: false }" class="relative">
                            <button @click="dropdownOpen = !dropdownOpen" 
                                ...
                                <div class="hidden lg:block text-left">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">超級管理員</div>
                                </div>
            ...
            document.querySelectorAll('form').forEach(function (form) {
                ...
                form.addEventListener('submit', function (e) {
                    ...
                    Object.keys(radioGroups).forEach(function (name) {
                        ...
                        if (container) { var err = container.querySelector('.field-error'); if (err) err.remove(); }
                        if (!checked) {
                            errors++;
                            var p = document.createElement('p');
                            p.className = 'field-error text-sm text-red-500 mt-1';
                            p.textContent = '請選擇一個選項';
                            group[group.length - 1].parentElement.after(p);
                            if (!firstError) firstError = group[0];
                        }
                    });
                    if (errors > 0) { e.preventDefault(); if (firstError) firstError.focus(); }
                });
                form.querySelectorAll('[required]').forEach(function (el) {
